...
class RolesController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				...
				roles.date_deleted LIKE ? OR 
				roles.is_deleted LIKE ?
			)";
			$search_params = array(
				"%$text%","%$text%","%$text%","%$text%"
			);
	...
	function view($rec_id = null, $value = null){
		...
		if($record){
			$page_title = $this->view->page_title = get_lang('view_roles');
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
	...
	function add($formdata = null){
		...
		$page_title = $this->view->page_title = get_lang('add_new_roles');
	...
	function edit($rec_id = null, $formdata = null){
		...
		$page_title = $this->view->page_title = get_lang('edit_roles');
		if(!$data){
			$this->set_page_error();
		}
